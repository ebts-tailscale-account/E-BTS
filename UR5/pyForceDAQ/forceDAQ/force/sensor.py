...
class Sensor(DAQReadAnalog):
    ...
    def determine_bias_fake(self, n_samples=100):
        ...
        """
        # print("1")
        save_path = Path("/home/rustam/nusense_code/pyForceDAQ/calibration/ati_bias.npz")
        if not save_path.exists():
            raise FileNotFoundError(f"Saved bias file not found: {save_path}")

        # Load mean vector
        with np.load(save_path) as z:
            mean_vec = z["mean"]

        # Apply to ATI DAQ if available
        if self._atidaq is not None:
            self._atidaq.bias(mean_vec)
            print(f"✅ Loaded and applied bias from {save_path}")
        else:
            print(f"⚠️ No ATI DAQ connected. Bias vector loaded but not applied.")
    ...
    def determine_bias(self, n_samples=100):
        ...
        # Save both raw samples and the mean for traceability
        np.savez_compressed(save_path, samples=data, mean=mean_vec)
