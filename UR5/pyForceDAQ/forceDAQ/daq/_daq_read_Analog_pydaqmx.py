...
class DAQReadAnalog(PyDAQmx.Task):
    ...
    def __init__(self, configuration, read_array_size_in_samples):
        ...
        print('init')
        print(f"Physical Channel: {configuration.physicalChannel}")
        try:
            PyDAQmx.Task.__init__(self)
            print("h")
        except Exception as e:
            print(f"Error: {e}")
        print("Checking terminal configuration: Diff")
        ...
        try:
            self.CreateAIVoltageChan(
                ...
                configuration.minVal,
                configuration.maxVal,
