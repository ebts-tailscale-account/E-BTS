...
class ProcessPriorityManager(object):
    ...
    def add_subprocess(self, process):
        ...
        # else:
        #     logging.warning(f"Process {process} has no PID attribute.")
        try:
            pid = process.pid
            self._subprocs.append(process)
            # print(f"{process} was added!")
        except AttributeError as e:
            logging.warning(f"AttributeError: {e}. Process: {process}")
        except Exception as e:
            logging.warning(f"Unexpected error: {e}. Process: {process}")
        except:
